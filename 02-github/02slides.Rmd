...
1. [Prologue and checklist](#prologue)
...
- Marcelo's materials, in turn, acknowledge and build on:
  - Grant McDermott's [course materials](https://github.com/uo-ec607/lectures) (Lecture 2), developed during his time at the University of Oregon.
  - Pedro Sant'Anna's [ECON 525 materials](https://psantanna.com/Econ520/) and slide theme at Emory University.

- I also benefited from Julia Wrobel's [BIOS 731 course materials](https://bios731.github.io/).
...
Before we continue, please confirm that you have:

☑ Created a [GitHub](https://github.com/) account using your Emory email address.

☑ Created a [DataCamp](https://app.datacamp.com/) account using your Emory email address.

☑ Downloaded and installed [VS Code](https://code.visualstudio.com/Download).

☑ Downloaded and installed [Git](https://git-scm.com/downloads).

---

# In-class check-in

Let's review the setup checklist together now.

☑ Complete the in-class assessment and submit your personal GitHub username.
...
<div align="center">
<img src="https://phdcomics.com/comics/archive/phd101212s.gif" height=500>
</div>
...
Let's open a terminal—or the integrated terminal in VS Code—and verify that Git is ready.

- **macOS:** Open **Terminal** (Applications → Utilities → Terminal).
- **Windows:** Open **PowerShell** or **Git Bash**.
- **VS Code:** Select **Terminal → New Terminal**.

```bash
git --version
```

If Git is installed, the output begins with `git version`.
...
- Share code and project materials online.
- Collaborate safely on a shared codebase.
- Experiment without risking stable work.
- Restore earlier versions when needed.
- Track what changed, when, and why.
- Practice a standard workflow in data science and research.
...
- **History:** The sequence of commits in a repository.
- **Local:** The repository copy on your computer.
- **Remote:** A repository hosted elsewhere, such as on GitHub.
...
The starting point for our workflow is to link a GitHub repository (i.e. "repo") to a VS Code project. Here are the steps we're going to follow:
...
Now, create a GitHub repository called "test" and clone it in VS Code by following these steps.
<div style="margin-top: 1.5cm;"></div>

See Marcelo's GIF walkthrough on the next slide (Mac users).

I'll demonstrate this on my computer; please follow along on yours.
...
- There should not be any files to commit at the moment.
...
Again, see Marcelo's GIF walkthrough on the next slide (Mac users).

We will do it together on my laptop; please follow along on yours.
...
<div style="margin-top: -.5cm;"></div>
1. **Stage** (or "add")
  - Select file additions, modifications, or deletions for the next commit.
2. **Commit**
  - Record the staged changes as a snapshot in the repository history.
3. **Pull**
  - Fetch and integrate changes from the configured upstream branch on GitHub.
4. **Push**
  - Upload committed changes from your local branch to GitHub.
...
<div style="margin-top: -.5cm;"></div>
- VS Code's Git integration and built-in GUI cover all the major operations.
...
- Stage all additions, modifications, and deletions across the working tree.
```bash
$ git add -A
```
- Stage modifications and deletions to tracked files, but not untracked files.
```bash
$ git add -u
```
...
# Main Git shell commands (cont.)
- Stage additions, modifications, and deletions under the current directory.
```bash
$ git add .
``` 
- Commit the currently staged changes with a descriptive message.
```bash
...
- Upload committed changes from the current local branch to its configured remote.
```bash
$ git push
```
...
```
Git has paused the merge so P1 can resolve the conflicting changes without accidentally overwriting either version of the README.
...
<div style="margin-top: 1.5cm;"></div>
Once that's done, stage the resolved file, commit the merge, and push the result to GitHub.
...
- Allow you to try out a whole new idea *without affecting* your `main` branch.
...
```
<div style="margin-top: 1cm;"></div>
Delete a fully merged local branch, then delete its remote counterpart:
```bash
...
<div style="margin-top: 1cm;"></div>
- Try this now. Use the [course repo](https://github.com/DataManagementVisual/Econ525-Fall2026) if you can't think of anyone else's.
...
- We'll talk more about [Markdown](https://www.markdownguide.org/) (and its close relation, [R Markdown](https://rmarkdown.rstudio.com/)) in the next lectures.
...
2. Clone the repo to your local machine, preferably using VS Code. (For example, from the shell: `$ git clone REPOSITORY-URL`.)

3. .hi[Stage all additions, modifications, and deletions:] `$ git add -A`
